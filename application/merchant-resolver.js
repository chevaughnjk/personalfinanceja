 ...
 *
 *  STRUCTURAL SINGLE-SOURCE ENFORCEMENT:
 *    This is the ONLY module permitted to import ../settings/merchant-intelligence.js.
 *    categorise.js, category-rules.js and read-statements.js must import THIS
 *    module's public surface (normalise / resolve) and never reach past it. A
 *    second implementation cannot form because there is no second entry point
 *    to build one from. (Back this with an eslint no-restricted-imports rule on
 *    merchant-intelligence.js for every path except this file.)
 *
 *  CONSTRUCTION RIGOUR:
...
import { compileMerchantIntelligence, resolveMerchant } from '../settings/merchant-intelligence.js';
...
const DEFAULT_INSTITUTION_SECTOR_RE =
  /financial\s*-\s*(bank|credit union|building society)/i;
...
function firstSegmentLower(s) {
  return String(s == null ? '' : s).split(',')[0].replace(/\s+/g, ' ').trim().toLowerCase();
}
...
export function createMerchantResolver(deps) {
  ...
  const fuzzy = Object.assign({ enabled: false, minScore: 0.9, reviewBelow: 0.95 }, options.fuzzy || {});
  ...
  function normalise(descriptor, opts = {}) {
    ...
    if (profile === 'card') {
      ...
      return { raw, cleaned: raw, matchName: firstSegmentLower(raw), identifiers: [] };
    }
  ...
  function emptyResolution() {
    return {
      merchant: null, canonicalName: null, brand: null, merchantGroup: null,
      category: null, sector: null, confidence: null, reviewRequired: false,
      matchType: 'none', score: 0, groupKey: null, displayLabel: null,
      incidentalInstitution: false, payee: null,
    };
  }
  ...
  function resolve(descriptor, opts = {}) {
    ...
    const stripInstitutions = opts.stripInstitutions == null ? (profile === 'bank') : !!opts.stripInstitutions;
  ...
  return Object.freeze({ normalise, resolve, compiled: merchants });
}
