// When a statement is imported the researched merchant list is checked first.
// resolveMerchant looks a descriptor up in that list; if it is a known merchant
// its category is used, otherwise the category rules below decide.
import { cutAtBranchHyphen } from './shared-helpers.js';
...
export function compileRules(categories) {
  ...
  const compiled = [];
  const flat = (p) => Array.isArray(p)
    ? p
    : [...((p && p['jamaica-pack']) || []), ...((p && p.universal) || [])];
  for (const cat of categories) {
    const words = flat(cat.patterns).map(String).filter((w) => w.trim());
    const headWords = ((!Array.isArray(cat.patterns) && cat.patterns && cat.patterns.head) || [])
      .map(String).filter((w) => w.trim());
    if (!words.length && !headWords.length) continue;
    // Reproduces the Python: (?<![a-z])(?:word1|word2|...) with IGNORECASE.
    const entry = { name: cat.name, re: words.length ? new RegExp('(?<![a-z])(?:' + words.join('|') + ')', 'i') : null };
    if (headWords.length) entry.headRe = new RegExp('(?<![a-z])(?:' + headWords.join('|') + ')', 'i');
    compiled.push(entry);
  }
  return compiled;
}
...
export function categorise(description, compiled, fallback = 'Uncategorised', resolver = null, refundHint = null) {
  ...
  if (resolver) {
    ...
    if (r.merchant) {
      ...
      return { category: r.category, confidence: level === 'low' ? 0.4 : 0.9, merchant: r.canonicalName, needsReview };
    }
  }
  ...
  if (refundHint && refundHint.isCredit && refundHint.refundCategory) {
    return { category: refundHint.refundCategory, confidence: 0.6, needsReview: true };
  }
